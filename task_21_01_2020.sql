...
v_query:='
    SELECT 
        f_name::text, E''/*\n''||CASE WHEN '||vtime||' THEN NOW() AT TIME ZONE ''Asia/Kolkata''::text||'':'' ELSE '' '' END||E'' Auto generated by get_function_ddl API v1\n*/\nCREATE OR REPLACE FUNCTION ''||f_name||''(''||
...
RETURN query
EXECUTE v_query;
v_query :=  'SELECT REPLACE(indexdef, ''INDEX'', ''INDEX IF NOT EXISTS'')||'';'' FROM pg_indexes WHERE tablename = 
...
v_query := '
    SELECT 
        table_name, get_table_ddl(table_name, '||vtime||') 
    FROM 
        (SELECT table_name::text FROM INFORMATION_SCHEMA.TABLES WHERE table_schema = '''||vschema||''')t;
';
RETURN query     
    EXECUTE v_query;
END;
...
v_query := '
    ...
    GROUP BY trigger_name, action_statement, action_timing, action_orientation)t_tow;
';
EXECUTE v_query INTO vresult;
RETURN vresult;
END;
$$ LANGUAGE plpgsql;
...
v_query := '
    SELECT 
        table_name, get_trigger_ddl(table_name, '||vtime||') 
    FROM (SELECT table_name::text FROM INFORMATION_SCHEMA.TABLES WHERE table_schema = '''||vschema||''')t;';
RETURN query     
EXECUTE v_query;
END;
$$ LANGUAGE plpgsql;
